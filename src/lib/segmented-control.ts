...
export const segmentedControlItemVariants = cva(
  [
    "relative inline-flex shrink-0 cursor-pointer items-center justify-center rounded-md border border-transparent text-base font-medium whitespace-nowrap text-muted-foreground/72 outline-2 outline-transparent transition-[outline-color,background-color,color] duration-(--duration-quick) ease-(--ease-smooth-out) select-none hover:bg-transparent hover:text-muted-foreground focus-visible:outline-ring disabled:pointer-events-none disabled:opacity-64 motion-reduce:transition-none sm:text-sm data-disabled:pointer-events-none data-disabled:opacity-64",
