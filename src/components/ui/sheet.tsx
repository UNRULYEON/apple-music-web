...
export function SheetBackdrop({
  ...
  return (
    <SheetPrimitive.Backdrop
      className={cn(
        "fixed inset-0 z-50 bg-black/32 backdrop-blur-sm transition-all duration-(--duration-fast) ease-(--ease-smooth-out) data-ending-style:duration-(--duration-quick) data-ending-style:opacity-0 data-starting-style:opacity-0",
        className,
      )}
    ...
    <SheetPortal {...portalProps}>
      ...
      <SheetViewport side={side} variant={variant}>
        <SheetPrimitive.Popup
          className={cn(
            "relative flex max-h-full min-h-0 w-full min-w-0 flex-col bg-popover not-dark:bg-clip-padding text-popover-foreground shadow-lg/5 transition-[opacity,translate] duration-(--duration-fast) ease-(--ease-smooth-out) data-ending-style:duration-(--duration-quick) will-change-transform before:pointer-events-none before:absolute before:inset-0 before:shadow-[0_1px_--theme(--color-black/4%)] data-ending-style:opacity-0 data-starting-style:opacity-0 max-sm:before:hidden dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
            side === "bottom" &&
